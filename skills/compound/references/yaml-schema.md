...
- **component**: Component or subsystem involved (project-specific string, e.g., `api`, `auth`, `database`, `frontend`, `backend`, `cli`, `worker`, `config`, `tests`, `docs`, `tooling`)
...
- **root_cause**: One of `missing_dependency`, `missing_import`, `missing_index`, `wrong_api`, `scope_issue`, `thread_violation`, `async_timing`, `memory_leak`, `config_error`, `logic_error`, `test_isolation`, `missing_validation`, `missing_permission`, `missing_workflow_step`, `inadequate_documentation`, `missing_tooling`, `incomplete_setup`, `type_error`, `compatibility_issue`
- **resolution_type**: One of `code_fix`, `migration`, `config_change`, `test_fix`, `dependency_update`, `environment_setup`, `workflow_improvement`, `documentation_update`, `tooling_addition`, `seed_data_update`, `schema_change`
...
- **framework_version**: Framework or runtime version in `X.Y` or `X.Y.Z` format
...
9. `framework_version`, if present, must match `X.Y` or `X.Y.Z` and only applies to bug-track docs.
