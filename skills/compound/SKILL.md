---
name: compound
description: Document a recently solved problem to compound your team's knowledge. Use when a problem is solved, a fix is verified, or after phrases like "that worked", "it's fixed", "working now", "problem solved".
---
...
1. Read MEMORY.md from the auto memory directory (the path is known from the system prompt context). If the auto memory directory path is not known, check `~/.claude/projects/` for the current project's memory directory, or skip this step.
...
#### 2. **Solution Extractor**
   ...
   **Bug track output sections:**
   ...
   - **Prevention**: Strategies to avoid recurrence, best practices, and test cases. Include concrete code examples where applicable (e.g., configurations, test assertions, linting rules)
...
#### 3. **Related Docs Finder**
   ...
   **Search strategy (grep-first filtering for efficiency):**
   ...
   3. Use the native content-search tool (e.g., Grep) to pre-filter candidate files BEFORE reading any content. Run multiple searches in parallel, case-insensitive, targeting frontmatter fields. These are template patterns -- substitute actual keywords:
      - `title:.*<keyword>`
...
The `compound-refresh` skill is **not** a default follow-up. Use it selectively when the new learning suggests an older learning or pattern doc may now be inaccurate.

It makes sense to load the `compound-refresh` skill when one or more of these are true:
...
- If there is **one obvious stale candidate**, load the `compound-refresh` skill with a narrow scope hint after the new learning is written
...
4. If not:
   ...
   b. Draft the smallest addition that communicates the three things. Match the file's existing style and density. The addition should describe the knowledge store itself, not the plugin -- an agent without the plugin should still find value in it.
      ...
      ```
   c. In full mode, explain to the user why this matters -- agents working in this repo (including fresh sessions, other tools, or collaborators) won't know to check `docs/solutions/` unless the instruction file surfaces it. Show the proposed change and where it would go, then use `AskUserQuestion` to get consent before making the edit. In compact-safe mode, output a one-liner note and move on
...
Based on problem type, optionally invoke specialized reviewers to enhance the documentation. Read the matching section from `references/phase3-reviewers.md` and dispatch via the Agent tool:

- **performance_issue** -> Performance Oracle
- **security_issue** -> Security Sentinel
- **database_issue** -> Data Integrity Guardian
- **test_failure** -> Test Reviewer
- Any code-heavy issue -> Code Quality Reviewer + Code Simplicity Reviewer
...
**Compact-safe output:**
```
OK Documentation complete (compact-safe mode)
...
Subagent Results:
  OK Context Analyzer: Identified performance_issue in brief_system, category: performance-issues/
  OK Solution Extractor: 3 code fixes, prevention strategies
  OK Related Docs Finder: 2 related issues

Specialized Agent Reviews (Auto-Triggered):
  OK Performance Oracle: Validated query optimization approach
  OK Code Quality Reviewer: Code examples meet conventions
  OK Code Simplicity Reviewer: Solution is appropriately minimal
  OK Documentation Style Editor: Documentation style verified
...
**After displaying the success output, present the "What's next?" options using `AskUserQuestion`.** Do not continue the workflow or end the turn without the user's selection.

**Alternate output (when updating an existing doc due to high overlap):**

```
OK Documentation updated (existing doc refreshed with current context)
...
## Applicable Specialized Reviewers

Based on problem type, these reviewers from `references/phase3-reviewers.md` can enhance documentation:

### Code Quality & Review
- **Code Quality Reviewer**: Reviews code examples for framework best practices
...
- **Documentation Style Editor**: Reviews documentation style and clarity
- **Framework Docs Researcher**: Links to framework/library documentation references

### When to Invoke
- **Auto-triggered** (optional): Reviewers can run post-documentation for enhancement
- **Manual trigger**: User can invoke reviewers after /compound completes for deeper review

## Related Workflows

- Use alongside planning and brainstorming workflows for comprehensive development
- `/compound-refresh` maintains and refreshes existing documentation as the codebase evolves
